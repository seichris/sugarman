...
- Status: P1/P2 and one-value already-active iPhone interoperability passed for one owned sensor on 2026-08-30; official Android handback also passed. The first managed Device Test run on 2026-09-01 reached the durably prepared history request, then failed closed before its CoreBluetooth write acknowledgement; Android handback again passed. Its generic diagnostics do not prove the triggering command. Five-reading durability, final `0x36` product meaning, zero-unsupported-command behavior, iPhone reconnect, timestamp parity, native state mapping, and sensor-index wrap remain incomplete. The production branch contains a host-testable ownership/reconnect/history coordinator, a typed known-peer foreground CoreBluetooth adapter, and an isolated normal-app Device Test target with Bluetooth-inert private import, an explicit scan-only bridge for the existing Probe JSON, and a separate process-local arm gate. The bridge holds the shared owner lease, retains only exact-name matches, fails closed on ambiguity, and cannot connect or use GATT. The inbound policy recognizes one exact checksum-valid 24-byte observed history preamble only in the sole pending-history-write window, emits a payload-free count, and preserves terminal handling for all duplicate, late, malformed, and other unsupported frames. The normal release bootstrap still installs no controller factory or active-session material. Decoded values remain `questionable` until state mapping is physically verified. A signed exact artifact and fresh physical confirmation still gate any execution. See [`GS3_FOREGROUND_PRODUCTION_DESIGN.md`](GS3_FOREGROUND_PRODUCTION_DESIGN.md), [`GS3_DEVICE_TEST_PROVISIONING.md`](GS3_DEVICE_TEST_PROVISIONING.md), and [`GS3_DEVICE_TEST_PHYSICAL_RESULT_2026-09-01.md`](GS3_DEVICE_TEST_PHYSICAL_RESULT_2026-09-01.md). Fresh activation remains unresolved.
- Date: 2026-08-28
