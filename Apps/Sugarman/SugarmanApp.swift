...
final class AppModel {
    ...
    private func normalizeWorkoutSelection() {
    ...
    func loadDemo(_ scenario: SyntheticDemoScenario) async throws {
        ...
        do {
            ...
            for plan in try await primaryStore.workoutPlans() {
                try await demoStore.insertWorkoutPlan(plan)
            }
            store = demoStore
    ...
    private func refreshFromStore() async throws {
        do {
            ...
            let newWorkoutPlans = try await store.workoutPlans()
